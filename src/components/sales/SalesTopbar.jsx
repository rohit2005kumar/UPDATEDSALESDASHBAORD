...
export default function SalesTopbar() {
  ...
  return <header className="sticky top-0 z-20 flex min-h-20 items-center gap-2 border-b border-line bg-[#fffefa]/95 px-3 backdrop-blur sm:gap-4 sm:px-5 lg:px-8">
    ...
    <div className="grid size-10 shrink-0 place-items-center rounded-full border-2 border-[#dfe4d3] bg-brand text-xs font-bold text-white">{name.split('')[0].toUpperCase()}</div>
    <div className="hidden xl:block"><div className="text-xs font-bold">{name}</div><div className="text-[10px] text-muted">{user.role}</div>
    </div>
